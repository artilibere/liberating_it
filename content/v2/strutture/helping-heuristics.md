---
slug: helping-heuristics
title: "Helping Heuristics: imparare ad aiutarsi a vicenda"
meta_description: "Le Helping Heuristics sono un metodo progressivo per migliorare il modo in cui diamo e riceviamo aiuto nelle organizzazioni, sperimentando diversi pattern "
registro: manuale-operativo
durata: "15 minuti"
difficolta: "Intermedia"
partecipanti: "gruppi da 3 persone"
fase: "Test"
complessita: "Per iniziare subito"
url: "https://liberating.it/structures/helping-heuristics/"
---

# Helping Heuristics

[Home](/) > [Le strutture](/structures/) > Helping Heuristics

**In breve** - Le Helping Heuristics sono un metodo progressivo per migliorare il modo in cui diamo e riceviamo aiuto nelle organizzazioni, sperimentando diversi pattern di interazione e offrendo preziosi spunti per sviluppare relazioni di aiuto più efficaci.

| Durata | Difficolta' | Gruppo | Fase |
|--------|-------------|--------|------|
| 15 minuti | Intermedia | gruppi da 3 persone | Test |

**Filtri:** [Per iniziare subito](/complessita/iniziare-subito/) · [Facile](/difficolta/facile/) · [Breve (max 45 min)](/durata/breve/) · [Test](/design-thinking/test/)

## Domanda da portare

- "Come possiamo migliorare il modo in cui offriamo e riceviamo aiuto per creare soluzioni più efficaci?"
- "Qual e' la sfida piu' urgente su cui lavorare adesso?"
- "Cosa cambieresti se avessi piu' autonomia in questo tema?"

## Cosa ti serve

- Spazio aperto senza tavoli.
- Partecipanti in piedi.
- Configurazione in triadi: cliente, coach e osservatore.
- Timer per gestire i round.
- Utilizzo di breakout rooms per le triadi.

## I passaggi

1. Presence il coach pratica l'ascolto compassionevole. - 2 min
2. Accetta tutte le offerte del cliente senza intervenire.
3. Discovery il coach guida attraverso domande. - 2 min
4. Focus sulla scoperta reciproca.
5. Round 3 , Loving Provocation il coach offre consigli mirati. - 2 min
6. Interviene quando nota elementi non visti dal cliente.

## Quando usarla

- Quando ridurre gli errori comuni quando si offre o si chiede aiuto.
- Quando modificare pattern disfunzionali nell'offrire e ricevere supporto.
- Quando sviluppare maggiore consapevolezza delle proprie abitudini relazionali .

## Il consiglio del facilitatore

Incoraggiare il cambio di ruoli tra i round.

## Errori da evitare

- Prestare attenzione al linguaggio non verbale.
- Non condividere il vero problema.

## Domande frequenti

### Cos'e' Helping Heuristics?
Le Helping Heuristics sono un metodo progressivo per migliorare il modo in cui diamo e riceviamo aiuto nelle organizzazioni, sperimentando diversi pattern di interazione e offrendo preziosi spunti per sviluppare relazioni di aiuto più efficaci.

### Quanto dura Helping Heuristics?
Di solito 15 minuti. I passaggi prevedono tempi fissi (2 min, 2 min, 2 min). Adatta i tempi al tuo gruppo, ma rispetta le transizioni.

### Quando conviene usare Helping Heuristics?
Quando ridurre gli errori comuni quando si offre o si chiede aiuto.

## Prima e dopo

- **Prima:** [1-2-4-All](/structures/1-2-4-all/) - formato base per coinvolgere tutti
- **Dopo:** [What, So What, Now What? (W³)](/structures/w3-what-so-what-now-what/) - W³) (capire cosa fare dopo

## Strutture simili

- [Impromptu Networking](/structures/impromptu-networking/) - facile e veloce
- [Troika Consulting](/structures/troika-consulting/) - facile, lavoro in piccolo gruppo

## Torna al catalogo

[Esplora tutte le strutture](/structures/) · [1-2-4-All](/structures/1-2-4-all/) - la struttura piu' semplice per iniziare
